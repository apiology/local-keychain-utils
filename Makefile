...
quicktypecheck: ## Use dmypy for cached mypy runs.
	@dmypy run --timeout 300 *.py tests local_keychain_utilities
...
typecheck: ## run mypy against project
	mypy --cobertura-xml-report typecover --html-report typecover local_keychain_utilities
	mypy tests
...
test: ## run tests quickly with the default Python
	@coverage run --source local_keychain_utilities -m pytest
...
docs: ## generate Sphinx HTML documentation, including API docs
	rm -f docs/local_keychain_utilities.rst
	rm -f docs/modules.rst
	sphinx-apidoc -o docs/ local_keychain_utilities
	$(MAKE) -C docs clean
	$(MAKE) -C docs html
...
release: dist ## package and upload a release
	...
	twine upload -u $$(with-op op get item 'PyPI - test' --fields username) -p $$(with-op op get item 'PyPI - test' --fields password) dist/local_keychain_utilities-$${new_version:?}.tar.gz -r testpypi; \
	twine upload -u $$(with-op op get item 'PyPI' --fields username) -p $$(with-op op get item 'PyPI' --fields password) dist/local_keychain_utilities-$${new_version:?}.tar.gz -r pypi
